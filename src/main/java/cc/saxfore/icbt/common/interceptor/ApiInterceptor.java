...
import cc.saxfore.icbt.common.annotation.ICApiURL;
...
public class ApiInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        ICApiURL apiURL = null;
        if (handler instanceof HandlerMethod) {
            HandlerMethod method = (HandlerMethod) handler;
            apiURL = method.getMethodAnnotation(ICApiURL.class);
        }

        if (apiURL != null) {
            String accessToken = request.getParameter("accessToken");
            if (ICStringUtil.isBlank(accessToken) || !specialToken.equalsIgnoreCase(accessToken)) {
                ICRespResult.fail(response, "您无权访问该地址！");
                return false;
            }
        }
